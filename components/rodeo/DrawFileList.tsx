...
export function DrawFileList({ files }: DrawFileListProps) {
  if (files.length === 0) {
    return <p className="text-sm text-body-text">No sheets posted yet.</p>;
  }
    ...
    <div className="flex flex-wrap gap-2">
      {files.map((file) => (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="flex items-center gap-2 text-sm px-3 py-1.5 rounded-md border border-border text-body-text hover:bg-highlight"
        >
          <span className="text-xs font-semibold text-body-text">
            {fileIcon(file.fileType)}
          </span>
          <span>{file.label}</span>
          <span className="text-xs text-body-text">({typeLabel[file.type]})</span>
        </a>
      ))}
    </div>
  );
